      ...
      <div class="col-lg-8">
        ...
        <form class="small" method="POST">
          <div class="form-floating mb-3">
            <input type="text" name="username" class="form-control <?php $form->echo_valid_class('username') ?>"
              id="username" placeholder="username" value="<?php echo $form->get_value('username') ?>">
            <label for="username">Username</label>
            <?php $form->echo_formatted_errors('username') ?>
          </div>
          <div class="form-floating mb-3">
            <input type="text" name="email" class="form-control <?php $form->echo_valid_class('email') ?>" id="email"
              placeholder="name@example.com" value="<?php echo $form->get_value('email') ?>">
            <label for="email">Email address</label>
            <?php $form->echo_formatted_errors('email') ?>
          </div>
          <div class="form-floating mb-3">
            <input type="date" name="dob" class="form-control <?php $form->echo_valid_class('dob') ?>" id="dob"
              value="<?php echo $form->get_value('dob') ?>">
            <label for="dob">Date of Birth</label>
            <?php $form->echo_formatted_errors('dob') ?>
          </div>
          <div class="form-floating mb-3">
            <input type="password" name="password"
              class="form-control <?php $form->echo_valid_class('password', true) ?>" id="password"
          ...
          <div class="form-floating mb-3">
            <input type="password" name="confirmPassword"
              class="form-control <?php $form->echo_valid_class('confirm_password', true) ?>" id="confirmPassword"
              placeholder="Password">
            <label for="confirmPassword">Confirm Password</label>
            <?php $form->echo_formatted_errors('confirm_password') ?>
          </div>
          <div class="form-check mb-1">
            <input class="form-check-input <?php $form->echo_valid_class('privacy_policy', true) ?>"
              name="privacyPolicy" type="checkbox" id="privacyPolicy">
            <label class="form-check-label" for="privacyPolicy">
              I agree to the <a href="/privacy-policy" target="register-rules">Privacy Policy</a>
            </label>
          </div>
          <div class="form-check mb-3">
            <input class="form-check-input <?php $form->echo_valid_class('eula', true) ?>" name="eula" type="checkbox"
              id="eula">
