...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if (isset($_POST['active_version'])) {
    ...
    $db = new Database;
    $db->beginTransaction();
    $db->delete('active_versions', 'platform_name = "mac"');
    if ($_POST['active_version'] != 'none') {
      $db->insert('active_versions', ['platform_name' => 'mac', 'version_id' => $_POST['active_version']]);
    }
    $db->commit();
  } else {
    require_once(__DIR__ . '/../../inc/services/Database.php');
    $db = new Database;
    $latest_version = $db->select('SELECT * FROM game_versions WHERE platform_name = "mac" ORDER BY version_number DESC LIMIT 1');
    $latest_version_num = $latest_version[0]['version_number'] ?? 0;
    $latest_version_num++;

    $upload_dir = __DIR__ . '/../../uploads/';
    $new_filename = "versions/dreamscape_mac-v$latest_version_num.zip";
...
require_once(__DIR__ . '/../../inc/services/Database.php');
$db = new Database;
$all_uploaded = $db->select('SELECT game_versions.id, version_number, file_name  FROM game_versions JOIN uploads ON game_versions.upload_id = uploads.id WHERE platform_name = "mac" ORDER BY version_number DESC');

$active_version = $db->select('SELECT version_id FROM active_versions WHERE platform_name = "mac"');
